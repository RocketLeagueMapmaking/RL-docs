<template>
    <span class="item-render-content">
        ...
        <div v-else class="child">
            <div>
                <span :title="`${item.Package}.${item.Class}`" style="padding-right: 8px;">{{ item.type }}</span>
                <span :title="item.category != null ? `Editor category: (${item.category})` : ''">{{ item.name }}</span>
            </div>
            <span v-if="item.replicated === 'True'" style="background-color: var(--accentColor); border-radius: 2px; padding: 1px 10px;">
                R
            </span>
        </div>
        <div class="actions" v-if="parents.length > 0">
            <button title="Share this property" @click="copy(shareUrl(), 'the item path')">
                <span class="iconify" data-icon="fa6-solid:link"></span>
            </button>
            <button title="Copy kismet" @click="fetchKismet()">
                <span class="iconify" data-icon="fa6-solid:clone"></span>
            </button>
        </div>
    </span>
</template>
